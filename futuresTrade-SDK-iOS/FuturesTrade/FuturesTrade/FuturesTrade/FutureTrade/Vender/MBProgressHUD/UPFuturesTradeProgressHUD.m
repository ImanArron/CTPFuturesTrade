...
#define UPFuturesTradeMainThreadAssert() NSAssert([NSThread isMainThread], @"UPFuturesTradeProgressHUD needs to be accessed on the main thread.");

CGFloat const UPFuturesTradeProgressMaxOffset = 1000000.f;

static const CGFloat UPFuturesTradeDefaultPadding = 4.f;
static const CGFloat UPFuturesTradeDefaultLabelFontSize = 16.f;
static const CGFloat UPFuturesTradeDefaultDetailsLabelFontSize = 12.f;


@interface UPFuturesTradeProgressHUD () {
    // Deprecated
...
@interface UPFuturesTradeProgressHUDRoundedButton : UIButton
@end


@implementation UPFuturesTradeProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    UPFuturesTradeProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    UPFuturesTradeProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (UPFuturesTradeProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            UPFuturesTradeProgressHUD *hud = (UPFuturesTradeProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = UPFuturesTradeProgressHUDAnimationFade;
    _mode = UPFuturesTradeProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)showAnimated:(BOOL)animated {
    UPFuturesTradeMainThreadAssert();
...
- (void)animateIn:(BOOL)animatingIn withType:(UPFuturesTradeProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == UPFuturesTradeProgressHUDAnimationZoom) {
        type = animatingIn ? UPFuturesTradeProgressHUDAnimationZoomIn : UPFuturesTradeProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == UPFuturesTradeProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == UPFuturesTradeProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == UPFuturesTradeProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == UPFuturesTradeProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    UPFuturesTradeProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<UPFuturesTradeProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    UPFuturesTradeBackgroundView *backgroundView = [[UPFuturesTradeBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = UPFuturesTradeProgressHUDBackgroundStyleSolidColor;
    ...
    UPFuturesTradeBackgroundView *bezelView = [UPFuturesTradeBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:UPFuturesTradeDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:UPFuturesTradeDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [UPFuturesTradeProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:UPFuturesTradeDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[UPFuturesTradeRoundProgressView class]];

    UPFuturesTradeProgressHUDMode mode = self.mode;
    if (mode == UPFuturesTradeProgressHUDModeIndeterminate) {
    ...
    else if (mode == UPFuturesTradeProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[UPFuturesTradeBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == UPFuturesTradeProgressHUDModeDeterminate || mode == UPFuturesTradeProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[UPFuturesTradeRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == UPFuturesTradeProgressHUDModeAnnularDeterminate) {
            [(UPFuturesTradeRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == UPFuturesTradeProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == UPFuturesTradeProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[UPFuturesTradeProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[UPFuturesTradeProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[UPFuturesTradeRoundProgressView class]]) {
        UPFuturesTradeRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [UPFuturesTradeRoundProgressView appearanceWhenContainedIn:[UPFuturesTradeProgressHUD class], nil];
#else
        appearance = [UPFuturesTradeRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[UPFuturesTradeProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((UPFuturesTradeRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((UPFuturesTradeRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[UPFuturesTradeBarProgressView class]]) {
        UPFuturesTradeBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [UPFuturesTradeBarProgressView appearanceWhenContainedIn:[UPFuturesTradeProgressHUD class], nil];
#else
        appearance = [UPFuturesTradeBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[UPFuturesTradeProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((UPFuturesTradeBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((UPFuturesTradeBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    UPFuturesTradeBackgroundView *bezelView = self.bezelView;
...
- (void)layoutSubviews {
    ...
    // would trigger a zoUPFuturesTradeie object access.
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? UPFuturesTradeDefaultPadding : 0.f;
...
- (void)setMode:(UPFuturesTradeProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == UPFuturesTradeProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation UPFuturesTradeRoundProgressView
...
@implementation UPFuturesTradeBarProgressView
...
@interface UPFuturesTradeBackgroundView ()
...
@implementation UPFuturesTradeBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = UPFuturesTradeProgressHUDBackgroundStyleBlur;
            ...
            _style = UPFuturesTradeProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(UPFuturesTradeProgressHUDBackgroundStyle)style {
    if (style == UPFuturesTradeProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = UPFuturesTradeProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    UPFuturesTradeProgressHUDBackgroundStyle style = self.style;
    if (style == UPFuturesTradeProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == UPFuturesTradeProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation UPFuturesTradeProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [UPFuturesTradeProgressHUD allHUDsForView:view];
    for (UPFuturesTradeProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable UPFuturesTradeProgressHUDCompletionBlock)completion {
...
- (BOOL)diUPFuturesTradeackground {
    UPFuturesTradeBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == UPFuturesTradeProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDiUPFuturesTradeackground:(BOOL)diUPFuturesTradeackground {
    UPFuturesTradeMainThreadAssert();
    self.backgroundView.style = UPFuturesTradeProgressHUDBackgroundStyleSolidColor;
    self.backgroundView.color = diUPFuturesTradeackground ? [UIColor colorWithWhite:0.f alpha:.2f] : [UIColor clearColor];
}
...
@implementation UPFuturesTradeProgressHUDRoundedButton
